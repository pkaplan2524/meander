...
router.get('/', async (req, res) => {
    try {
        ...
        if (req.query && req.query.org) {
            filter['org'] = new ObjectId(req.query.org) 
        }

         if (req.query && req.query.owner) {
            filter['owner'] = new ObjectId(req.query.owner) 
        }
       
       const record = await Model.find(filter, "name active").collation({ locale: "en" }).sort(sort)
        if (!record) {
            return res.status(404).send()
        }
